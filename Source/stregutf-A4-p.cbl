      ...
      *****************************************************************
       ...
       77  mov-kg-1-A               pic 9(12)v999.
       77  mov-kg-1-D               pic 9(12)v999.
       77  mov-kg-2-A               pic 9(12)v999.
       77  mov-kg-2-D               pic 9(12)v999.
       77  mov-kg-3-A               pic 9(12)v999.
       77  mov-kg-3-D               pic 9(12)v999.
       ...
       MAIN-PRG.
           ...
           if tutto-ok
              ...
              if tutto-ok
                 ...
                 if not trovato
                    evaluate true
                    when stregutf-cumulativo
                         display message 
                                 "Nessun movimento UTF da elaborare!"
                                   title titolo
                                    icon 2
                    when stregutf-serie-1
                         display message 
                          "Nessuno scarico bolle serie 1 da elaborare!"
                                   title titolo
                                    icon 2
                    when stregutf-serie-2
                         display message 
                          "Nessuno scarico bolle serie 2 da elaborare!"
                                   title titolo
                                    icon 2
                    end-evaluate
      *****           else
      ...
      ***---
       ELABORAZIONE.
           ...
           if not trovato-giorno
              move low-value to mov-rec
              move stregutf-data-da(1:4) to mov-anno
              move utf-reg-serie-1       to mov-num-reg
              move save-data             to mov-data
              set mov-uscita             to true
              start movutf key is >= k-data-reg
                    invalid continue
                not invalid
                    read movutf next 
                         at end continue
                     not at end
                     if mov-anno    = stregutf-data-da(1:4) and
                        mov-data    = save-data
                        if mov-num-reg = utf-reg-serie-1 or
                           mov-num-reg = utf-reg-serie-2 or
                           mov-num-reg = utf-reg-serie-3
      ...
      ***---
       SCRIVI-INTESTAZIONE.
           ...
           move r-tit-movim-data to line-riga.
           write line-riga.            
           add 1 to Rowcounter.
      ...
      ***---
       USCITE-SERIE-1-2.
           move 0 to mov-kg-1-A mov-kg-1-D.
           move low-value             to mov-rec.
           move stregutf-data-da(1:4) to mov-anno.
           move utf-reg-serie-1       to mov-num-reg.
           move save-data             to mov-data.
           ...
           if tutto-ok
              perform until 1 = 2
                 ...
                 if mov-num-reg not = utf-reg-serie-1
                    exit perform
                 end-if
                 if mov-data > save-data
                    exit perform
                 ...
                 if mov-col-A
                    add mov-kg to mov-kg-1-A
                 end-if
                 if mov-col-D
                    add mov-kg to mov-kg-1-D
                 end-if

              end-perform
           end-if.

           move 0 to mov-kg-2-A mov-kg-2-D.
           move low-value             to mov-rec.
           move stregutf-data-da(1:4) to mov-anno.
           move utf-reg-serie-2       to mov-num-reg.
           move save-data             to mov-data.
           set mov-uscita             to true.
      *****     set  trovato-giorno        to false.
           start movutf key is >= k-data-bolla
           ...
           if tutto-ok
              perform until 1 = 2
                 ...
                 if mov-num-reg not = utf-reg-serie-2
                    exit perform
                 end-if
                 if mov-data > save-data
                    exit perform
                 ...
                 if mov-col-A
                    add mov-kg to mov-kg-2-A
                 end-if
                 if mov-col-D
                    add mov-kg to mov-kg-2-D
                 end-if

              end-perform
           end-if.

           move 0 to mov-kg-3-A mov-kg-3-D.
           move low-value             to mov-rec.
           move stregutf-data-da(1:4) to mov-anno.
           move utf-reg-serie-3       to mov-num-reg.
           move save-data             to mov-data.
           set mov-uscita             to true.
      *****     set  trovato-giorno        to false.
           start movutf key is >= k-data-bolla
           ...
           if tutto-ok
              perform until 1 = 2
                 ...
                 if mov-num-reg not = utf-reg-serie-3
                    exit perform
                 end-if
                 if mov-data > save-data
                    exit perform
                 ...
                 if mov-col-A
                    add mov-kg to mov-kg-3-A
                 end-if
                 if mov-col-D
                    add mov-kg to mov-kg-3-D
                 end-if
           ...
           set mov-uscita to true.
           move 0 to mov-cod-clifor.
           move 0 to mov-num-doc.
           if mov-kg-1-A not = 0
              move mov-kg-1-A to mov-kg
              move "TOT SCARICO SERIE BOLLE 1 COL. A"
                to cli-ragsoc-1
              set mov-col-A to true
              perform VALORIZZA-RIGA
           end-if.
           if mov-kg-1-D not = 0
              move mov-kg-1-D to mov-kg
              move "TOT SCARICO SERIE BOLLE 1 COL. D"
                to cli-ragsoc-1
              set mov-col-D to true
              perform VALORIZZA-RIGA
           end-if.
           if mov-kg-2-A not = 0
              move mov-kg-2-A to mov-kg
              move "TOT SCARICO SERIE BOLLE 2 COL. A"
                to cli-ragsoc-1
              set mov-col-A to true
              perform VALORIZZA-RIGA
           end-if.
           if mov-kg-2-D not = 0
              move mov-kg-2-D to mov-kg
              move "TOT SCARICO SERIE BOLLE 2 COL. D"
                to cli-ragsoc-1
              set mov-col-D to true
              perform VALORIZZA-RIGA
           end-if.
           if mov-kg-3-A not = 0
              move mov-kg-3-A to mov-kg
              move "TOT SCARICO SERIE BOLLE 3 COL. A"
                to cli-ragsoc-1
              set mov-col-A to true
              perform VALORIZZA-RIGA
           end-if.
           if mov-kg-3-D not = 0
              move mov-kg-3-D to mov-kg
              move "TOT SCARICO SERIE BOLLE 3 COL. D"
                to cli-ragsoc-1
              set mov-col-D to true
              perform VALORIZZA-RIGA
           end-if.
      ...
      ***---
       VALORIZZA-RIGA.
           ...
           initialize r-riga.
           |Non aumento il progressivi per le righe di riepilogo
           if mov-cod-clifor not = 0
      ...
      ***---
       SCRIVI-TOTALI.
           ...
           if utf-cumulativo
              ...
              move "TOTALI GIORNO"           to r-ragsoc
              move tot-mov-giorno-c-bianchi  to r-c-bianchi
              move tot-mov-giorno-s-bianchi  to r-s-bianchi
              move tot-mov-giorno-c-bianchi  to r-c-lubrif
              move tot-mov-giorno-s-lubrif   to r-s-lubrif
           end-if.
           move r-riga                    to line-riga.
           write line-riga.
           add 1 to RowCounter.
       ...
       SCRIVI-RIPORTO.
           ...
           if prima-volta
              if utf-cumulativo
                 ...
                 move "RIPORTO USCITE PRECEDENTI"   to r-ragsoc
                 move utf-prog-u-bianchi   to r-s-bianchi
                                                    tot-bianchi-scarico
                 move utf-prog-u-lubrif    to r-s-lubrif
                                                       tot-altri-scarico
      ...
      ***---
       SCRIVI-REGISTRO.
           ...
           move utf-num-reg-ass to st-reg.
           move st-t-reg to line-riga.
           write line-riga.          
           add 1 to Rowcounter.
      ...
      ***---
       SALTO-PAGINA.
           perform SCRIVI-RIPORTO.
           if Rowcounter <= 78-MaxRows
              compute diff-righe = 78-MaxRows - RowCounter
              add 1 to diff-righe giving n-vuote
              perform RIGHE-VUOTE
           end-if.                               
           move 0 to Rowcounter.
           perform SCRIVI-REGISTRO.
