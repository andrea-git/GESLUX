      ...
      *    i movimenti di magazzino)
           ...
           when "tgruppi"
                perform PREPARA-TGRUPPI           |CERCA
           when "tvettori"
           when "tvettori-alfa"
                perform PREPARA-TVETTORI           |CERCA
           when "tparameva"
           when "tparameva-alfa"
                perform PREPARA-TPARAMEVA          |SELEZIONA
           when "destini"
           when "destini-alfa"
           ...
           when "tmp-ordf-art"
           when "tmp-ordf-art2"
                perform PREPARA-TMP-ORDF-ART
           when "tmp-mtordini"
                perform PREPARA-TMP-MTORDINI
           when "tcaumag"
                perform PREPARA-TCAUMAG            |SELEZIONA TCAUMAG
           when "tcaumag-mag"
                perform PREPARA-TCAUMAG-MAG
           when "tcaumag-imp-neg-mag"
                perform PREPARA-CAUSALI-IMP-NEG-MAG
           when "tcaumag-imp-pos-mag"
                perform PREPARA-CAUSALI-IMP-POS-MAG
           when "tcaumag-si-mov"
           ...
           when "tmovmag-clifor"
                perform PREPARA-TMOVMAG-CLIFOR
           when "tcaumag-forn"
      ...
      *    visualizzare tutti i record indipendentemente dallo status
           ...
           when "articoli-frn"
                set TuttiIRec to true
                perform PREPARA-ARTICOLI-FRN     |CERCA
           when "MAS"
           ...
           when "impforn"
                perform PREPARA-IMPFORN             |CERCA

           when "mto-numordcli"
                perform PREPARA-MTO-NUMORDCLI       |CERCA
           when "tmp-tpromo-zoom"
                perform PREPARA-TMP-TPROMO-ZOOM
           when "zoom-bolle"
                perform PREPARA-ZOOM-BOLLE
           when "zoom-art-giac"
           ...
           when other
                display message box "PARAGRAFO INESISTENTE"
                exit program
           end-evaluate.
      ...
      ***---
       PREPARA-TCODPAG.
           ...
           move  16                      to xzoom-lw.
           move  90                      to xzoom-sw.
           move "TBLPA"                  to xzoom-file-name(1).
           move  0                       to xzoom-file-key.
      ...
      ***---
       PREPARA-TBLCO.
           ...
           move  16                      to xzoom-lw.
           move  90                      to xzoom-sw.
           move "TBLCO"                  to xzoom-file-name(1).
           move  0                       to xzoom-file-key.
      ...
      ***---
       PREPARA-TIVAESE.
           ...
           move "TBLIV"                  to xzoom-file-name(1).
      ...
      * CAMPO 7
           add 1 to idx
           move  2                       to xzoom-field-length(idx).
           move  443                     to xzoom-field-offset(idx).
           move  6                       to xzoom-field-column(idx).
      ...
      *    tutti i record indipendentemente dallo status
           if not TuttiIRec
              ...
              move "A" to como-record(3188:1)

              set xzoom-when-true(2)    to true
              set xzoom-begin-with(2)   to true
              set xzoom-ignore-case(2)  to true
                                 
              move como-record(3188:1)   to xzoom-wild-value(2)
              move 1                    to xzoom-wild-value-length(2)
              move 1                    to xzoom-wild-length(2)
              move 3188                 to xzoom-wild-offset(2)
           end-if
      ...
      * CAMPO 6
           ...
           if como-file = "ttipocli-GDO"
              move "S" to como-record(34:1)

              set xzoom-when-true(1)    to true
              set xzoom-begin-with(1)   to true 
              set xzoom-ignore-case(1)  to true
                                 
              move como-record(34:1)  to xzoom-wild-value(1)
              move 1                  to xzoom-wild-value-length(1)
              move 1                  to xzoom-wild-length(1)
              move 33                 to xzoom-wild-offset(1)
           end-if.
      ...
      ***---
       PREPARA-TGRUPPI.
           ...
           move  16                      to xzoom-lw.
           move  80                      to xzoom-sw.
           move "tgruppi"                to xzoom-file-name(1).
           move  0                       to xzoom-file-key.
           move  2                       to xzoom-fields.
      ...
      ***---
       PREPARA-TPARAMEVA.
           ...
           move  16                      to xzoom-lw.
           move  82                      to xzoom-sw.
           move "tparameva"              to xzoom-file-name(1).
           move  2                       to xzoom-fields.
      ...
      * CAMPO 2
           ...
           move  3                       to xzoom-field-offset(idx).
           move  52                      to xzoom-field-column(idx).
           move "Descrizione"            to xzoom-field-name(idx).  
           set  xzoom-ft-alpha(idx)      to true. 

           move  -1                     to xzoom-delimiter-offset.
           move  3                      to xzoom-delimiter-length.
           move "000"                   to xzoom-from-value.
           move "000"                   to xzoom-to-value.
      ...
      * CAMPO 7
           ...
           if not TuttiIRec
              ...
              move como-record(3188:1)  to xzoom-wild-value(idx)
              move 1                    to xzoom-wild-value-length(idx)
              move 1                    to xzoom-wild-length(idx)
              move 3188                 to xzoom-wild-offset(idx)
           end-if.
      ...
      ***---                 
       PREPARA-CLIENTI-DI-DESTINI.
           ...
           when "clienti-des-alf-A"
                ...
                move 1   to xzoom-wild-value-length(idx)
                move 1   to xzoom-wild-length(idx)
                move 877 to xzoom-wild-offset(idx)
      ...
      *    Definizione del campo 10
           ...
           move 0                           to xzoom-field-rel(Idx).
           move 1                           to xzoom-field-length(idx).
           move 492                         to xzoom-field-offset(idx).
      ...
      ***---
       PREPARA-PROGMAG-TMAGAZ.
           ...
           move 1                       to xzoom-wild-value-length(2).
           move 1                       to xzoom-wild-length(2).
           move 486                     to xzoom-wild-offset(2).
      ...
      * QUESTO FILTRO SCARTA IL RECORD PADRE...
           ...
           if como-file = "prg-artico-sons-a"
              ...
              move 492                  to xzoom-wild-offset(4)
           end-if.
      ...
      ***---
       PREPARA-TCAUMAG-MAG.
           ...
           |FILTRO SU MAGAZZINO
           set xzoom-when-true(idx)     to true.
           set xzoom-contains(idx)      to true.
           set xzoom-ignore-case(idx)   to false.
      *                                
           move como-record(89:3)       to xzoom-wild-value(idx).
           move 1                       to xzoom-wild-value-length(idx).
           move 1                       to xzoom-wild-length(idx).
           move 88                      to xzoom-wild-offset(idx).

      ***---
       PREPARA-CAUSALI-IMP-POS-MAG.
           perform PREPARA-TCAUMAG.
           
           move 1 to idx.
           |FILTRO SU IMPONIBILE POSITIVO
           set xzoom-when-true(idx)     to true.
           set xzoom-begin-with(idx)    to true.
           set xzoom-ignore-case(idx)   to false.
      *                                
           move "+"                     to xzoom-wild-value(idx).
           move 1                       to xzoom-wild-value-length(idx).
           move 1                       to xzoom-wild-length(idx).
           move 58                      to xzoom-wild-offset(idx).
      ...
      ***---
       PREPARA-CAUSALI-IMP-NEG-MAG.
           perform PREPARA-TCAUMAG. 
           
           move 1 to idx.
           |FILTRO SUL MAGAZZINO
           set xzoom-when-true(idx)     to true.
           set xzoom-equal(idx)         to true.
           set xzoom-ignore-case(idx)   to false.
      *                                
      ...
      ***---
       PREPARA-TMOVMAG-CLIFOR.
           ...
           move como-record(25:4)       to xzoom-wild-value(1).
           move 4                       to xzoom-wild-value-length(1).
           move 4                       to xzoom-wild-length(1).
           move 24                      to xzoom-wild-offset(1).
      ...
      * CAMPO 3
           add  1 to Idx.
           move 8                           to xzoom-field-length(idx).
           move 24                          to xzoom-field-offset(idx).
           move "Data Ordine"               to xzoom-field-name(idx).
           set xzoom-field-unsigned(Idx)    to true.
           set xzoom-ft-display(Idx)        to true.
           move 10                          to xzoom-field-digits(Idx).
      ...
      ** CAMPO 9
           add  1 to Idx.
           move 8                           to xzoom-field-length(idx).
           move 145                         to xzoom-field-offset(idx).
      ...
      ***---
       PREPARA-ARTICOLI-FRN.
           perform PREPARA-ARTICOLI.
           
           |FILTRO SULLA MARCA
           move como-record(2460:15) to como-file.
           if como-record(2460:15) not = spaces
              add 1 to idx
              move como-record(2460:15)  to xzoom-wild-value(idx)
              move 15                    to xzoom-wild-value-length(idx)
              move 15                    to xzoom-wild-length(idx)
              move 2459                  to xzoom-wild-offset(idx)
              set xzoom-when-true(idx)   to true
              set xzoom-begin-with(idx)  to true
              set xzoom-ignore-case(idx) to false
           end-if.
      ...
      *    Definizione del campo 2
           add  1                           to Idx.
           move 40                          to xzoom-field-length(idx).
           move 23                          to xzoom-field-offset(idx).
           move "Descrizione"               to xzoom-field-name(idx).  
           move 29                          to xzoom-field-column(Idx).
           set  xzoom-ft-alpha(idx)         to true. 
      ...
      *    Definizione del campo 4
           add  1                           to Idx.
           move 50                          to xzoom-field-length(idx).
           move 63                          to xzoom-field-offset(idx).
           move "Descrizione"               to xzoom-field-name(idx).  
           move 20                          to xzoom-field-column(Idx).
           set  xzoom-ft-alpha(idx)         to true. 
      ...
      *    Definizione del campo 6
           add   1                          to Idx.
           move  50                         to xzoom-field-length(idx).
           move  113                        to xzoom-field-offset(idx).
           move  20                         to xzoom-field-column(idx).
           move "Descrizione"               to xzoom-field-name(idx).  
           set  xzoom-ft-alpha(idx)         to true. 
      ...
      *       Definizione del campo 8
              add   1                        to Idx
              move  8                        to xzoom-field-length(idx)
              move  163                      to xzoom-field-offset(idx)
              move  9                        to xzoom-field-column(idx)
              move 8                         to xzoom-field-digits(Idx)
              move "##.###.##0"              to xzoom-field-fmt(Idx)
              set xzoom-al-right(Idx)        to true
      ...
      *       Definizione del campo 9
              add   1                        to Idx
              move  8                        to xzoom-field-length(idx)
              move  171                      to xzoom-field-offset(idx)
              move  9                        to xzoom-field-column(idx)
      ...
      ***---
       PREPARA-TORDFORN.
           ...
           move  zero                       to xzoom-cln.
           move  16                         to xzoom-lw.
           move  140                        to xzoom-sw.
           move "tordforn"                  to xzoom-file-name(1).
           move "tcaumag"                   to xzoom-file-name(2).
           move "destinif"                  to xzoom-file-name(3).
      ...
      *    File Reference Settings - Relazione tordforn > tcaumag
           move   1                to Idx.
           move   1                to xzoom-ref-m-file (Idx).
           move   2                to xzoom-ref-s-file (Idx).
           move   0                to xzoom-ref-s-key  (Idx).
      ...
      *    File Reference Settings - Relazione tordforn > destinf
           move   2                to Idx.
           move   1                to xzoom-ref-m-file (Idx).
           move   3                to xzoom-ref-s-file (Idx).
           move   0                to xzoom-ref-s-key  (Idx).
      ...
      *            Master File Settings
           move 10  to xzoom-ref-m-length(Idx, 1).
           move 32  to xzoom-ref-m-offset (Idx, 1).
           set xzoom-ref-m-unsigned (Idx, 1) to true.
           set xzoom-ref-m-alpha    (Idx, 1) to true.
      ...
      *            Slave File Settings
           ...
           |FILTRO SU RECORD ATTIVI
              ...
              move como-record(282:1)   to xzoom-wild-value(idx)
              move 1                    to xzoom-wild-value-length(idx)
              move 1                    to xzoom-wild-length(idx)
              move 281                  to xzoom-wild-offset(idx)
           end-if.

           if como-file = "tordforn-forn"
              move 1 to idx
              set xzoom-when-true(idx)    to true
              set xzoom-begin-with(idx)   to true
              set xzoom-ignore-case(idx)  to true
                                 
              move como-record(33:5)    to xzoom-wild-value(idx)
              move 5                    to xzoom-wild-value-length(idx)
              move 5                    to xzoom-wild-length(idx)
              move 32                   to xzoom-wild-offset(idx)
           end-if.
      ...
      *    Definizione del campo 2
           ...
           move  8                          to xzoom-field-length(Idx).
           move  4                          to xzoom-field-offset(Idx).
           move "Ordine"                    to xzoom-field-name(Idx).  
           move 10                          to xzoom-field-column(Idx).
           move 8                           to xzoom-field-digits(Idx).
           move 0                           to xzoom-field-dec(Idx).
           move "#######0"                  to xzoom-field-fmt(Idx).
      ...
      ***---
       PREPARA-IMPFORN.
           ...
           move  16                      to xzoom-lw.
           move  80                      to xzoom-sw.
           move "impforn"                to xzoom-file-name(1).
           move  2                       to xzoom-fields.
      ...
      ***---
       PREPARA-MTO-NUMORDCLI.
           ...
           move  16                         to xzoom-lw.
           move  132                        to xzoom-sw.
           move "mtordini"                  to xzoom-file-name(1).
           move "clienti"                   to xzoom-file-name(2).
           move "destini"                   to xzoom-file-name(3).
      ...
      * CAMPO 6
           ...
           move 1                           to xzoom-field-length(idx).
           move 4                           to xzoom-field-column(idx).
           move 709                         to xzoom-field-offset(idx).
           move "Stato"                     to xzoom-field-name(idx).  
           set xzoom-ft-alpha(idx)          to true.              
           set xzoom-al-center(Idx)         to true.
           ...
           move  -1                         to xzoom-delimiter-offset.
           move  5                          to xzoom-delimiter-length.
           move "00000"                     to xzoom-from-value.
           move "zzzzz"                     to xzoom-to-value.

           |FILTRO SU NUM ORD CLI
           set xzoom-when-true(1)    to true.
           set xzoom-begin-with(1)   to true.
           set xzoom-ignore-case(1)  to true.
                              
           move como-record(28:10)   to xzoom-wild-value(1).
           move 1                    to xzoom-wild-value-length(1).
           move 1                    to xzoom-wild-length(1).
           move 27                   to xzoom-wild-offset(1).

           move 0 to idx.
      *    File Reference Settings - Relazione mtordini > clienti
           add    1                to Idx.
           move   1                to xzoom-ref-m-file (Idx).
           move   2                to xzoom-ref-s-file (Idx).
           move   0                to xzoom-ref-s-key  (Idx).
      ...
      *    File Reference Settings - Relazione mtordini > destini
           add    1                to Idx.
           move   1                to xzoom-ref-m-file (Idx).
           move   3                to xzoom-ref-s-file (Idx).
           move   0                to xzoom-ref-s-key  (Idx).
      ...
      *            Master File Settings
           move 10  to xzoom-ref-m-length(Idx, 1).
           move 17  to xzoom-ref-m-offset (Idx, 1).
           set xzoom-ref-m-unsigned (Idx, 1) to true.
           set xzoom-ref-m-alpha    (Idx, 1) to true.
      ...
      ***---
       PREPARA-TMP-MTORDINI.
           initialize xzoom-linkage xzoom-ext-info(1).

           move  0                      to xzoom-row.
           move  0                      to xzoom-cln.
           move  16                     to xzoom-lw.
           move  148                    to xzoom-sw.
      ...
      *    Definizione del campo 3
           ...
           move 12                          to xzoom-field-offset(idx).
           move 8                           to xzoom-field-column(idx).
           move "G.D.O."                    to xzoom-field-name(idx).  
           set  xzoom-ft-alpha(idx)         to true. 
      ...
      *    Definizione del campo 6
           ...
           move 8                           to xzoom-field-length(idx).
           move 117                         to xzoom-field-offset(idx).
           move "Dt Ordine"                 to xzoom-field-name(idx).
           set xzoom-field-unsigned(Idx)    to true.
           set xzoom-ft-display(Idx)        to true.
           move 8                           to xzoom-field-digits(Idx).
           move 0                           to xzoom-field-dec(Idx).
      ...
      *    Definizione del campo 7
           ...
           move 8                           to xzoom-field-length(idx).
           move 125                         to xzoom-field-offset(idx).
           move "Dt Cons."                  to xzoom-field-name(idx).
           set xzoom-field-unsigned(Idx)    to true.
           set xzoom-ft-display(Idx)        to true.
           move 8                           to xzoom-field-digits(Idx).
           move 0                           to xzoom-field-dec(Idx).
      ...
      *    Definizione del campo 8
           ...
           move  8                          to xzoom-field-length(Idx)
           move  133                        to xzoom-field-offset(Idx)
           move "Quantità"                  to xzoom-field-name(Idx)
           move 7                           to xzoom-field-column(Idx)
           move 8                           to xzoom-field-digits(Idx)
           move 0                           to xzoom-field-dec(Idx)
           move "##.###.###0"               to xzoom-field-fmt(Idx)
      ...
      *    Definizione del campo 9
           add  1 to Idx.
           move 15                          to xzoom-field-length(idx).
           move 142                         to xzoom-field-offset(idx).
           move "STATO"                     to xzoom-field-name(idx).  
           move 12                          to xzoom-field-column(Idx).
           set  xzoom-ft-alpha(idx)         to true. 
           set xzoom-al-center(Idx)         to true.
      ...
      *    Definizione del campo 4
           ...
           move 40                          to xzoom-field-length(idx).
           move 17                          to xzoom-field-offset(idx).
           move 33                          to xzoom-field-column(idx).
           move "Fornitore"                 to xzoom-field-name(idx).  
           set  xzoom-ft-alpha(idx)         to true. 
      ...
      *    Definizione del campo 7
           add   1                          to Idx
           move  8                          to xzoom-field-length(Idx)
           move  80                         to xzoom-field-offset(Idx)
           if como-file = "tmp-ordf-art2" 
              move "Evadere"                to xzoom-field-name(Idx)
           else
              move "Evasa"                  to xzoom-field-name(Idx)           
           end-if
           move 8                           to xzoom-field-column(Idx)
      ...
      *    Definizione del campo 8
           ...
           move  8                          to xzoom-field-length(Idx)
           move  88                         to xzoom-field-offset(Idx)
           move "Prevista"                  to xzoom-field-name(Idx)
           move 8                           to xzoom-field-column(Idx)
           move 8                           to xzoom-field-digits(Idx)
           move 0                           to xzoom-field-dec(Idx)
           move "##.###.###0"               to xzoom-field-fmt(Idx)
      ...
      *    Definizione del campo 9
           ...
           move 8                           to xzoom-field-length(idx).
           move 96                          to xzoom-field-offset(idx).
           move "Dt Arrivo"                 to xzoom-field-name(idx).
           set xzoom-field-unsigned(Idx)    to true.
           set xzoom-ft-display(Idx)        to true.
           move 8                           to xzoom-field-digits(Idx).
           move 0                           to xzoom-field-dec(Idx).
      ...
      *    Definizione del campo 4
           ...
           move  0                       to xzoom-field-rel(Idx).      
           move  8                          to xzoom-field-length(Idx).
           move  70                         to xzoom-field-offset(Idx).
           move "Data inizio"               to xzoom-field-name(Idx).  
           set xzoom-field-unsigned(Idx)    to true.
           set xzoom-ft-display(Idx)        to true.
           move 8                          to xzoom-field-digits(Idx).
           move   0                         to xzoom-field-dec(Idx).
      ...
      *    Definizione del campo 5
           ...
           move  0                       to xzoom-field-rel(Idx).      
           move  8                          to xzoom-field-length(Idx).
           move  78                         to xzoom-field-offset(Idx).
           move "Data fine"                 to xzoom-field-name(Idx).  
           set xzoom-field-unsigned(Idx)    to true.
           set xzoom-ft-display(Idx)        to true.
           move 8                           to xzoom-field-digits(Idx).
           move   0                         to xzoom-field-dec(Idx).
      ...
      * CAMPO 5
           ...
           move  68                         to xzoom-field-offset(idx).
           move  10                         to xzoom-field-column(idx).
           move "Giacenza LBX"              to xzoom-field-name(idx).  
           move 8                           to xzoom-field-digits(Idx).
           move 0                           to xzoom-field-dec(Idx).
           move "##.###.##0"                to xzoom-field-fmt(Idx).
      ...
      ***---
       PREPARA-PROMO.
           ...
           move  16                         to xzoom-lw.
           move  155                        to xzoom-sw.
           move "tpromo"                    to xzoom-file-name(1).
           move "tgrupgdo"                  to xzoom-file-name(2).
      ...
      * CAMPO 5
           ...
           move 8                           to xzoom-field-length(idx).
           move 20                          to xzoom-field-offset(idx).
           move "DPO dal"                   to xzoom-field-name(idx).
           set xzoom-field-unsigned(Idx)    to true.
           set xzoom-ft-display(Idx)        to true.
           move 10                          to xzoom-field-digits(Idx).
           move   0                         to xzoom-field-dec(Idx).
      ...
      * CAMPO 6
           ...
           move 8                           to xzoom-field-length(idx).
           move 28                          to xzoom-field-offset(idx).
           move "DPO al"                    to xzoom-field-name(idx).
           set xzoom-field-unsigned(Idx)    to true.
           set xzoom-ft-display(Idx)        to true.
           move 10                          to xzoom-field-digits(Idx).
           move   0                         to xzoom-field-dec(Idx).
      ...
      * CAMPO 7
           add  1 to Idx.
           move 8                           to xzoom-field-length(idx).
           move 87                          to xzoom-field-offset(idx).
           move "Volantino dal"             to xzoom-field-name(idx).
           set xzoom-field-unsigned(Idx)    to true.
           set xzoom-ft-display(Idx)        to true.
           move 10                          to xzoom-field-digits(Idx).
           move   0                         to xzoom-field-dec(Idx).
      ...
      * CAMPO 8
           ...
           move 8                           to xzoom-field-length(idx).
           move 95                          to xzoom-field-offset(idx).
           move "Volantino al"              to xzoom-field-name(idx).
           set xzoom-field-unsigned(Idx)    to true.
           set xzoom-ft-display(Idx)        to true.
           move 10                          to xzoom-field-digits(Idx).
           move   0                         to xzoom-field-dec(Idx).
           ...
           move  -1                         to xzoom-delimiter-offset.
           move  15                         to xzoom-delimiter-length. 
           move "000"                       to xzoom-from-value.
           move "000"                       to xzoom-to-value.

      *    File Reference Settings - Relazione tpromo > tgrupgdo
           move   1                to Idx.
           move   1                to xzoom-ref-m-file (Idx).
           move   2                to xzoom-ref-s-file (Idx).
           move   0                to xzoom-ref-s-key  (Idx).
      ...
      *            Slave File Settings
           ...
           |FILTRO SUL GRUPPO GDO
           if como-record(16:5) not = spaces
              set xzoom-when-true(1)    to true
              set xzoom-begin-with(1)   to true
              set xzoom-ignore-case(1)  to true
