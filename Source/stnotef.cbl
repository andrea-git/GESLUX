      ...
      ***---
       NOTEF-ERR SECTION.
           ...
           when "35"
                display message "Impossibile procedere."
                  x"0d0a""File note [NOTEF] inesistente"
                          title titolo
                           icon 2
                set errori to true
           when "39"
      ...
      ***---
       OPEN-FILES.
           open output lineseq.
           if tutto-ok
              open input notef clienti destini
           end-if.

      ***---
       ELABORAZIONE.
           move stampa-codice-da  to notf-codice.
           move stampa-destino-da to notf-prog.
           start notef key is >= notf-chiave
                 invalid  set errori to true
           end-start.
           if tutto-ok
              move 0 to cli-codice
              perform until 1 = 2
                 read notef next at end            exit perform end-read
                 if notf-codice > stampa-codice-a  exit perform end-if  
                 set record-ok to true
                 if notf-codice not = cli-codice
                    move notf-codice to cli-codice
                    set cli-tipo-F  to true
                    read clienti no lock invalid continue end-read
                 end-if
                 move notf-chiave to des-chiave
                 read destini no lock 
                      invalid initialize des-rec 
                 end-read
                 if stampa-tipo-cli not = spaces and
                    stampa-tipo-cli not = cli-prov
                    set record-ok to false
                 end-if
                 if record-ok
                    if stampa-cod-gdo not = spaces and
                       stampa-cod-gdo not = cli-nazione
                       set record-ok to false
                    end-if
                 end-if
                 if record-ok
      ...
      ***---
       GENERA-FILE-EXCEL.
           ...
           if notf-codice not = save-codice
              move notf-codice to save-codice
              initialize line-riga
              move notf-codice      to r-codice
              initialize line-riga
           ...
           inspect notf-note-1   replacing trailing spaces by low-value.
           initialize r-note-data.
           string notf-data(7:2) delimited size
                  "/"           delimited size
                  notf-data(5:2) delimited size
                  "/"           delimited size
                  notf-data(1:4) delimited size
                  into r-note-data
           end-string.
           ...
           string separatore    delimited size
                  separatore    delimited size     
                  notf-note-1    delimited low-value
                  " "           delimited size
                  r-note-data   delimited size
                  separatore    delimited size
                  notf-note-2    delimited size
                  into line-riga
           end-string.
           write line-riga.
           ...
           string " "           delimited size
                  ...
                  " "           delimited size
                  separatore    delimited size
                  notf-note-3    delimited size
                  separatore    delimited size
                  notf-note-4    delimited size
                  into line-riga
           end-string.
           write line-riga.
           write line-riga from spaces.
      ...
      ***---
       GENERA-FILE-TXT.
           ...
           move notf-note-2      to r-note-2.
           write line-riga    from riga-3.
           add 1 to WrittenRows.

           move notf-note-3      to r-note-3.
           move notf-note-4      to r-note-4.
      ...
      ***---
       CLOSE-FILES.
           close notef lineseq clienti destini.
