       ...
       PROGRAM-ID.                      aggordfor-p.
       AUTHOR.                          Andrea.
       REMARKS. Motore per la lavorazione sul file ORDFOR. Se in linkage
                ...
                Le modifiche importanti sulle quantità vanno riportate
                anche nel programma qta-vend
      ...
      ******************************************************************
       ...
       FILE-CONTROL.
           ...
           copy "tcaumag.sl".
           copy "timbalqta.sl".
           |Per recuperare ordine/nota credito e sapere se viene da una promo
           copy "tordini.sl".
           copy "rordini.sl".
           copy "tnotacr.sl".
           copy "rnotacr.sl".
       ...
       MAIN-PRG.
           ...
           if tutto-ok
              if aggiorna-mese-corrente
                 ...
                 close ordfor
                 if mese-elab = 12
                    call   "azzordfor-p" using scr-oper-handle
                    cancel "azzordfor-p"
                 end-if
                 open input ordfor
              end-if
      ...
      ***---
       INIT.
           ...
           move link-data(5:2) to mese-elab.
       ...
       ELABORAZIONE.
           ...
           if tutto-ok
              ...
              perform until 1 = 2
                 ...
                 if tmo-cliente or tca-riordino-si
                    if tca-cliente    or tca-riordino-si
LUBEXX                 if tca-si-stat or tca-riordino-si
                          if tca-movim-ven-pos      or
                             tca-movim-ven-neg      or
                             tca-movim-resi-for-pos or
                             tca-movim-resi-for-neg or
                             tca-riordino-si
...
LUBEXX                       if tca-cod-magaz not = "EXD"
...
LUBEXX                 end-if
      ...
      *****        move rmo-codmag to ord-mag
              ...
              read ordfor 
                   ...
                   move link-user          to ord-utente-creazione
                   accept ord-ora-creazione  from time
                   accept ord-data-creazione from century-date
               not invalid
                   move link-user    to ord-utente-ultima-modifica
                   accept ord-ora-ultima-modifica  from time
                   accept ord-data-ultima-modifica
                     from century-date
              end-read
              if CurrentYear  add como-qta to ord-qta-corr-m(mese)
              else            add como-qta to ord-qta-past-m(mese)
              end-if
              write ord-rec invalid rewrite ord-rec end-write
              set trovato to true
           end-if.
