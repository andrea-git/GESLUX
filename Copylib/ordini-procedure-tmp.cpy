      ...
      ***---
       VALORIZZA-CELLE-CAMPI-MANUALE.
           ...
           inspect imb-descrizione  replacing trailing spaces 
                                              by low-value.
           inspect gror-des-imballo replacing trailing spaces 
                                              by low-value.
           move gror-qta-imballi    to imballi-ed.
           call "C$JUSTIFY" using imballi-ed, "L"
           initialize lab-art-buf.
BLISTR     if gror-blister = 1
BLISTR        string art-descrizione  delimited low-value
BLISTR               " - "            delimited size
BLISTR               gror-des-imballo delimited low-value
BLISTR               " ("             delimited size
...
BLISTR        end-string
              ...
              string  art-descrizione  delimited low-value
                      " - "            delimited size
                      gror-des-imballo delimited low-value
                      " da "           delimited size
           ...
           inspect gror-des-imballo replacing trailing 
                                    low-value by spaces.
           inspect art-descrizione  replacing trailing 
                                    low-value by spaces.
           move lab-art-buf         to col-des.
           move prg-peso-utf        to gror-peso-utf.
           move prg-peso-non-utf    to gror-peso-non-utf.
           move "N"                 to gror-omaggio.
      ...
      *****     if SaveGdo not = spaces and NoAssortimento
...
LABLAB        if tcl-si-recupero  exit paragraph end-if.
      ...
      *****        move ef-des-buf     to rmo-destino    convert
...
LUBEXX                 end-if
LUBEXX*****                       move rmo-imp-cons to ror-imp-consumo
LUBEXX*****                       move rmo-coubat   to ror-imp-cou-cobat
                    end-if
              end-start.
      *****     end-if.
...
LABLAB     if tcl-si-recupero
      ...
      *****           move art-prezzo-vendita to hid-prezzo
                 set  gror-bloccato      to true
                 move 0                  to gror-promo
              else
                 move rpr-codice         to ror-promo
                 move rpr-prz-acq        to ef-uni-buf gror-prezzo
                                            ror-prz-unitario
                                            ror-prz-commle
                 move rpr-codice         to gror-promo
              end-if
           else
              if trovato-movim
LUBEXX           if ttipocli-gdo
...
LUBEXX           end-if
                 ...
                 move 9999999,99 to ef-uni-buf
                                      ror-prz-unitario
                 move art-prezzo-vendita to gror-prezzo
              end-if
           end-if.
      *****        end-if
      *****     end-if. 
...
LUBEXX*****           if trovato-movim
LUBEXX*****              move rmo-imp-cons to ef-cons-buf col-cons ror-imp-consumo
LUBEXX*****              move rmo-coubat   to ef-cou-buf  col-cou ror-imp-cou-cobat
LUBEXX*****           else
LUBEXX*****     end-if.
...
LUBEXX*****           end-if.
      ...
      ***---
       IMPOSTE.
           ...
           if SiAssortimento
              ...
              evaluate true
              when ttipocli-estero   perform CALCOLO-IMPOSTE-ESTERO
      ...
      ***---
       CALCOLO-IMPOSTE-ESTERO.
           move 0 to ror-imp-consumo ror-imp-cou-cobat.
       ...
       IMBALLI-QTA.
           ...
           when mb-yes
                |MODIFICA RICHIESTA DA TRIVELLA 080306
                 move gror-des-imballo to link-des
...
BLISTR        end-string
      ...
      ***--- 
       VALUTA-CAMBIO-PREZZO.
           ...
           evaluate true
           when ror-prz-unitario = hid-prezzo continue
      ...
      ***---
       SPOSTAMENTO-2.           
           ...
           if riga not = event-data-2
              ...
              move hid-ordinato  to prg-ordinato
              perform LABEL-VALORI
...
LUBEXX                end-if
      ...
      *****             end-if
                ...
                if chk-num-ord-cli not = spaces
                   ...
                   move ef-data-buf to como-data
                   perform DATE-TO-FILE
                   move como-data to chk-data-ordine
                   ...
                   if chk-ord-status = -1
                      display message "Esiste già l'ordine con"
                               ...
                               x"0d0a""avente la stessa data e "
                               x"0d0a""lo stesso numero ordine."
                               x"0d0a""Proseguire?"
                                title titolo
                                 type mb-yes-no
                               giving scelta
           ...
           when 78-ID-ef-pag
                ...
                if ef-pag-buf not = spaces
                   ...
                   set errori to true        
                   display message "Inserimento codice "
                                   "pagamento mancante"
                           title = tit-err
                           icon mb-warning-icon
                   move 78-ID-ef-pag to control-id
      ...
      *****                prg-giacenza  not = hid-giacenza   or
      *****                prg-impegnato not = hid-impegnato  or
      *****                prg-ordinato  not = hid-ordinato   or NewRow
                   ...
                   if errori                                         
                      ...
                      when art-bloccato
                           display message "Articolo BLOCCATO"
                                   ...
                                   icon mb-warning-icon
                      when art-disattivo
                           display message "Articolo DISATTIVO"
                                   title = tit-err
                                   icon mb-warning-icon
                      when other
...
LUBEXX                   display message "Impossibile attribuire un "
LUBEXX                                   "prezzo con IVA omaggio"
...
LUBEXX             end-if
      ...
      *****          if SaveGDO = spaces
                ...
                if key-status = 13
                   ...
                   if ror-prz-unitario = 0 |Articolo omaggio
                      move 0 to ef-cons-buf ef-cou-buf col-cons col-cou
                      display ef-cons ef-cou
                   end-if
      ...
      ***---
       PULISCI-CAMPI-LABELS.
           ...
           move zero   to ef-art-buf  ef-cons-buf    
           ...
           display ef-art  ef-cons   ef-qta-oma
                   ef-cou  ef-sconto
                   ef-qta  ef-cod-iva
                   ef-uni  ef-imp
           ...
           move 0 to prg-giacenza prg-ordinato prg-impegnato 
                     lab-ivato-buf.
           perform LABEL-VALORI.
           display lab-giacenza lab-impegnato lab-ordinato
...
LUBEXX            from environment "CONTROLLO_SCOSTAMENTO".
      ...
      ***---
       CERCA.
           ...
           when 78-ID-ef-cau
                ...
                call "zoom-gt"  using como-file, zoom-tca-rec                                giving stato-zoom
                end-call
                cancel "zoom-gt"
                if stato-zoom = 0
      ...
      *****          end-if
           ...
           when 78-ID-ef-uni
LABLAB          if tcl-si-recupero and volantino-forzato = 0
           ...
           perform until 1 = 2
              ...
              if prg-cod-magazzino not = spaces and
                 ...
                 if record-ok
                    ...
                    move prg-ordinato        to tmp-prg-z-ordinato
...
LUBEXX        move tor-cod-cli to link-cli-codice
LUBEXX        call   "calfido" using calfido-linkage
LUBEXX        cancel "calfido"
LUBEXX        if ( saldo + effetti-rischio + Sum ) > cli-fido
LUBEXX           set errori to true
LUBEXX           compute como-numero = 
LUBEXX                   cli-fido    - ( saldo + effetti-rischio + Sum )
...
LUBEXX     end-if.
      ...
      ***--- 
       CONTROLLA-TOTALE.
           ...
           perform varying store-riga from 2 by 1 
                     until store-riga > tot-righe
              inquire form1-gd-2(store-riga, 5), cell-data in SavePrezzo
      ...
      ***---
       CONTA-CODICI-IVA.
           ...
           perform varying riga from 2 by 1
                     until riga > tot-righe
              set trovato to false
              inquire form1-gd-2(riga, 78-NumCol + 15), 
                      hidden-data in hid-cod-iva    
              perform varying idx from 1 by 1
                        until idx > 3
                 if hid-cod-iva = cod-iva(idx)
                    set trovato to true
                    exit perform
                 end-if
              end-perform
              if not trovato
                 ...
                 perform varying idx from 1 by 1
                    ...
                    if cod-iva(idx) = spaces
                       move hid-cod-iva to cod-iva(idx)
                       exit perform
                    end-if
                 end-perform
              end-if
           ...
           if ins-codice not = 0
              ...
              perform varying idx from 1 by 1 
                 ...
                 if trovato
                    ...
                    move prg-ordinato  to hid-ordinato
                    ...
                    if bli-el-perce(idx) not = 0
                       ...
                       if not trovato-promo
                          ...
                          compute como-prezzo =
                                  rpr-prz-acq * bli-el-perce(idx) / 100
                          move rpr-codice         to ror-promo
                          move como-prezzo        to ef-uni-buf 
                                                     hid-prezzo
                                                     ror-prz-unitario
                       ...
                       move 0 to ror-prz-unitario hid-prezzo
                                 col-uni
                                 ror-imp-consumo 
                                 col-cons
                                 ror-imp-cou-cobat 
                    ...
                    move art-peso-utf     to hid-utf
                    move art-peso-non-utf to hid-non-utf
